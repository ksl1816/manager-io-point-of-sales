...
- **Receipts** — on-screen receipt preview, thermal-style print layout showing the actual business name/address, and a genuine PDF download generated client-side via jsPDF, with an embedded Urdu/Arabic font so item or customer names in that script render as real (if unjoined/isolated-form) glyphs instead of garbled symbols
...
| Cash vs bank detection | Manager's account form has no cash/bank field and the API's `isBankAccount` flag is unreliable (true even for a plain cash drawer), so account type is inferred from bank-only features: **"Can have pending transactions"** enabled or an **IBAN** → Bank; neither → Cash |
...
- **Accounts Receivable account resolution.** Every Receipt, Payment, and Journal Entry that touches a customer's balance needs the correct AR control account. The extension resolves this, in order: (1) the customer's own configured control account, (2) an AR account already used on an existing Receipt for that customer, (3) an AR account used on *any* existing Receipt, (4) the Chart of Accounts' flagged `isAccountsReceivable` account via `balance-sheet-account-batch` / `control-account-batch`. **There is no hardcoded fallback account.** If all four lookups come back empty, the transaction is **stopped and an error is shown** rather than posting to a fixed UUID that might be wrong for your business — a clear failure you can act on beats silently bad ledger data. (A transient network error during the lookup is not cached, so it can be retried.) If your business has multiple AR accounts across customer groups, still double-check the first few transactions land in the right one.
- **Cash vs bank account classification.** The Day Register's cash figures only count your physical cash drawer, so the extension has to tell cash accounts from bank accounts. Manager's account form exposes no cash/bank field, and the API's `isBankAccount` flag is not dependable (it returns true even for a plain "Cash in Hand" account), so the type is inferred from bank-only features: an account with **"Can have pending transactions"** enabled or an **IBAN** is treated as Bank; an account with neither is treated as Cash. Recommended: in Manager, tick "Can have pending transactions" on real bank accounts and leave it unticked on the cash drawer. A genuine bank account with neither an IBAN nor pending transactions enabled would be read as cash until you enable one of those.
...
- **PDF receipts with Urdu/Arabic text render each letter in isolated form, not joined cursive Nastaliq script.** jsPDF has no Arabic text-shaping engine, so while the embedded font produces correct, readable Arabic-script glyphs (fixing what would otherwise be garbled symbols), it can't reproduce the connected letterforms of proper Urdu typesetting. Fine for legibility; not calligraphically accurate.
