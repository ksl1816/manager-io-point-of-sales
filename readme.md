...
| Cash vs bank detection | The physical cash-drawer account(s) are chosen **explicitly by the user** in ⚙ Setup → Cash Drawer Accounts and saved in the browser (localStorage). Only receipts/payments into those accounts count as cash. There is **no guessing fallback**: if none are selected, the Day Register does not compute a cash figure — it shows “Please configure your Cash Drawer Accounts first.” |
...
- **Cash vs bank is an explicit choice, with no guessing.** The Day Register's cash figures only count your physical cash drawer, so you designate which account(s) are the drawer in ⚙ Setup → **Cash Drawer Accounts** (saved in the browser). Manager's account form has no cash/bank field, and the API's `isBankAccount` flag is not dependable (it returns true even for a plain "Cash in Hand" account), so nothing is inferred: if no drawer accounts are selected, the Day Register refuses to show a cash figure and instead asks you to configure them first. This guarantees a genuine bank account can never be counted as cash by accident.
