...
- **Printable Day Register summary** — an 80mm thermal-formatted summary of the day (sales, receipts, returns, net cash movement, with each invoice's customer and Due/Credit status), with the business name and address pulled live from Manager's Business Details
...
6. From the receipt screen: **print**, **save as PDF** (a real PDF file, generated in-browser), **💳 Apply Credit** (if the customer has an existing credit balance elsewhere), or **💰 Record Payment** — which opens a multi-line payment form where Tendered drives Amount and Change automatically
7. Click **🗄 Register** to open the Day Register for any date — review that date's Sales Invoices (with an expandable Receipts breakdown per invoice) and Returns, record receipts, and **🖨 Print Day Summary** when done
